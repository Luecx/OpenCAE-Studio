...
def test_mesh_definition_ownership_contract_is_unambiguous():
    """Protect the graph shape that prevents duplicate definition ownership."""
    mesh_fields = {item.name for item in fields(MeshState)}
    block_fields = {item.name for item in fields(ElementBlock)}

    assert "element_definitions" in mesh_fields
    assert "elements" not in mesh_fields
    assert "definition_ref" in block_fields
    assert "definition" not in block_fields
