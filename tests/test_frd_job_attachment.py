"""Regression coverage for the FRD-only stored-result contract."""
...
def test_finish_analysis_attaches_only_frd(tmp_path, monkeypatch):
    output_base = tmp_path / "results"
    output_base.with_suffix(".res").write_text("temporary\n", encoding="utf-8")
    ...
    assert adapter.result_candidates(output_base) == [output_base.with_suffix(".frd")]

    job_manager_analysis.finish_analysis(manager, job.id, adapter, output_base, 0)

    assert selected == [output_base.with_suffix(".frd")]
