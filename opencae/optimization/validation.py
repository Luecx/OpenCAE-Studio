...
def validate_topology_optimization(project, optimization, *, build_operators=False):
    errors: list[str] = []
    index = None
    masks = {}
    try:
    ...
    if analysis is None:
        ...
        if len(steps) != 1 or steps[0].step_type != "Linear Static":
            errors.append("Topology optimization requires exactly one Linear Static step")
    ...
    if np.any(masks.get("frozen_solid", False) & masks.get("frozen_void", False)):
        errors.append("Frozen Solid and Frozen Void regions overlap")
    if np.any(masks.get("frozen_solid", False) & ~masks.get("design", False)):
        errors.append("Frozen Solid must be contained in the design domain")
    if np.any(masks.get("frozen_void", False) & ~masks.get("design", False)):
    ...
    if not np.any(free_design):
        errors.append("The design domain contains no free optimization elements")

    if len(optimization.objectives) != 1:
        errors.append("Create exactly one optimization objective")
    if len(optimization.filters) != 1:
        errors.append("Topology optimization requires exactly one filter definition")
    if len(optimization.controls) != 1:
        errors.append("Topology optimization requires exactly one controls definition")
    ...
    if objective is None:
        ...
        if objective_response not in optimization.responses:
            errors.append("The objective references a response outside this optimization")
        elif objective_response.response_type != ResponseType.STIFFNESS_ENERGY:
            errors.append("The current OC optimizer requires a Stiffness Energy objective")
        else:
            try:
                objective_mask = index.mask_for(project, objective_response.region)
                masks[objective_response.id] = objective_mask
                if not np.all(objective_mask):
                    errors.append(
                        "Stiffness Energy currently requires the complete exported model region"
                    )
            except Exception as exc:
                errors.append(f"Objective response: {exc}")

    active_constraints = [item for item in optimization.constraints if item.active]
    if len(active_constraints) != 1:
        errors.append("The OC optimizer currently requires exactly one active resource constraint")
    else:
        ...
        if response not in optimization.responses:
            errors.append("The active constraint references a response outside this optimization")
        elif response.response_type not in _RESOURCE_TYPES:
            errors.append("The active constraint must use Volume, Volume Fraction, Mass or Mass Fraction")
        else:
            try:
                masks[response.id] = index.mask_for(project, response.region)
            except Exception as exc:
                errors.append(f"Constraint response: {exc}")
            if response.response_type in {ResponseType.MASS, ResponseType.MASS_FRACTION}:
                selected = masks.get(response.id, np.zeros(index.count, dtype=bool))
                if np.any(selected & (~np.isfinite(index.material_densities) | (index.material_densities <= 0.0))):
                    errors.append("Mass responses require positive material density on every selected element")
        if constraint.operator != ConstraintOperator.LESS_EQUAL:
            errors.append("The current OC optimizer supports only <= resource constraints")
    ...
    if not 0.0 < controls.minimum_density < 1.0:
        errors.append("Minimum density must be greater than 0 and less than 1")
    if not controls.minimum_density <= controls.initial_density <= 1.0:
        errors.append("Initial density must lie between minimum density and 1")
    ...
    for symmetry in optimization.symmetries:
        ...
        if symmetry.symmetry_type == SymmetryType.PLANAR:
            if kind not in {"face", "datum_plane"}:
                errors.append(f"{symmetry.name}: planar symmetry requires a face or Datum Plane")
            else:
        ...
        if symmetry.symmetry_type == SymmetryType.ROTATIONAL:
            if kind not in {"edge", "datum_vector"}:
                errors.append(f"{symmetry.name}: rotational symmetry requires an edge or Datum Vector")
            else:
...
def _validate_planar_reference(symmetry, errors):
    ...
    normal = np.asarray(reference.get("normal") or reference.get("direction") or (), dtype=float)
    origin = np.asarray(reference.get("origin") or reference.get("point") or (), dtype=float)
    if normal.shape != (3,) or origin.shape != (3,) or np.linalg.norm(normal) <= 1.0e-14:
        errors.append(f"{symmetry.name}: the symmetry plane has no valid origin and normal")
        return
...
def _validate_axis_reference(symmetry, errors):
    reference = symmetry.reference
    direction = np.asarray(reference.get("direction") or (), dtype=float)
    origin = np.asarray(reference.get("origin") or reference.get("point") or (), dtype=float)
    if direction.shape != (3,) or origin.shape != (3,) or np.linalg.norm(direction) <= 1.0e-14:
        errors.append(f"{symmetry.name}: the rotation axis has no valid origin and direction")
        return
    points = np.asarray(reference.get("points") or (), dtype=float)
    if len(points) < 2:
        return
    unit = direction / np.linalg.norm(direction)
    radial = points - origin - ((points - origin) @ unit)[:, None] * unit[None, :]
    scale = max(float(np.linalg.norm(np.ptp(points, axis=0))), 1.0)
    if float(np.max(np.linalg.norm(radial, axis=1))) > scale * 1.0e-6:
        errors.append(f"{symmetry.name}: rotational symmetry requires a straight edge")
