...
class AnalysisController:
    ...
    def create_step(self, analysis_type):
        ...
        step = AnalysisStep(
            ...
            load_refs=[] if analysis_type == "Eigenfrequency" else [EntityRef.of(item, "Load") for item in self.store.project.loads],
            support_refs=[EntityRef.of(item, "Support") for item in self.store.project.supports],
            number_of_modes=10,
        )
        analysis = create_analysis(analysis_type, name=name, steps=[step])
        self.store.mutate(f"Created step {name}", lambda project: project.analyses.append(analysis))
        self.store.select(step)
        self.edit_step(step)

    def edit_step(self, step):
        dialog = StepDialog(
            step,
            self.store.project.loads,
            self.store.project.supports,
            self.parent,
            [analysis.steps[0].name for analysis in self.store.project.analyses if analysis.steps],
        )
        ...
        def apply(_project):
            ...
            step.number_of_modes = values["number_of_modes"]
            step.load_refs = [EntityRef(entity_id, "Load") for entity_id in values["load_ids"]]
            step.support_refs = [EntityRef(entity_id, "Support") for entity_id in values["support_ids"]]
            analysis = self._analysis_for(step)
            if analysis and analysis.name == old:
                analysis.name = step.name
    ...
    def reorder_steps(self):
        ...
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        order = dialog.order()
        self.store.mutate("Reordered steps", lambda project: project.analyses.sort(key=lambda analysis: order.index(analysis.steps[0].name)))

    def manage_collectors(self):
        steps = [analysis.steps[0] for analysis in self.store.project.analyses if analysis.steps]
        if not steps:
            return
        dialog = StepCollectorsDialog(steps, self.store.project.loads, self.store.project.supports, self.parent)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
    ...
    def _analysis_for(self, step):
        return next((analysis for analysis in self.store.project.analyses if any(item.id == step.id for item in analysis.steps)), None)
