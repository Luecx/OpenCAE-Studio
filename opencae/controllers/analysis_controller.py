...
class AnalysisController:
    ...
    def _repair_active_analysis(self, *_):
        ...
        if not isinstance(current, Analysis):
            self.active_analysis_id = self.store.project.analyses[0].id if self.store.project.analyses else ""

    def create_step(self, step_type: StepType | str):
        step_type = StepType.coerce(step_type)
        project = self.store.project
        name = next_name_from_names(step_type, [step.name for step in project.steps])
        step = AnalysisStep(
            name=name,
            step_type=step_type,
            load_refs=([] if step_type is StepType.EIGENFREQUENCY else [EntityRef.of(item, "Load") for item in project.loads]),
            support_refs=[EntityRef.of(item, "Support") for item in project.supports],
            number_of_modes=10,
        )
        self.store.add_entity(f"Created step {name}", project.id, "steps", step)
    ...
    def edit_step(self, step):
        ...
        if not isinstance(stored, AnalysisStep):
            ...
            return
        dialog = StepDialog(stored, project.loads, project.supports, self.parent, [item.name for item in project.steps])
        ...
        replacement.load_refs = [EntityRef(entity_id, "Load") for entity_id in values["load_ids"]]
        replacement.support_refs = [EntityRef(entity_id, "Support") for entity_id in values["support_ids"]]
        self.store.replace_entity(f"Edited step {old_name}", project.id, "steps", replacement)
    ...
    def reorder_steps(self):
        ...
        if len(project.steps) < 2:
            return
        dialog = StepReorderDialog([item.name for item in project.steps], self.parent)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        ...
        self.store.execute("Reordered steps", UpdateFieldCommand(project.id, "steps", before, after))

    def manage_collectors(self):
        project = self.store.project
        if not project.steps:
            return
        dialog = StepCollectorsDialog(project.steps, project.loads, project.supports, self.parent)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        values = dialog.values()
        ...
        self.store.execute("Updated step collectors", UpdateFieldCommand(project.id, "steps", deepcopy(project.steps), after))
    ...
    def new_analysis(self):
        ...
        value = Analysis(
            name=next_name_from_names("Analysis", [item.name for item in project.analyses]),
            solver=solver,
            deck_profile_id=(self.settings.default_deck_profile_id(solver, adapter) if adapter is not None else "builtin:femaster"),
        )
    ...
    def _analysis_dialog(self, value, current):
        project = self.store.project
        dialog = AnalysisDialog(value, project.steps, self._solver_adapters(), self.settings, existing_names=[item.name for item in project.analyses], parent=self.parent)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        candidate = dialog.result()
        if current is None:
            self.store.add_entity(f"Created Analysis {candidate.name}", project.id, "analyses", candidate)
        else:
            self.store.replace_entity(f"Edited Analysis {candidate.name}", project.id, "analyses", candidate)
    ...
    def run_active(self):
        ...
        if self.jobs is None:
            ...
            return
        dialog = RunAnalysisDialog(analysis, self._solver_adapters(), self.settings, self.parent)
        if dialog.exec() != QDialog.DialogCode.Accepted:
            return
        ...
        if solver != analysis.solver or profile_id != analysis.deck_profile_id:
            ...
            self.store.replace_entity(f"Updated run configuration for {analysis.name}", self.store.project.id, "analyses", candidate)
