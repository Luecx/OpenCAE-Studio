...
class PartDatums:
    ...
    def datum_point(self):
        self._open("Point", DatumPointDialog)

    def datum_vector(self):
        self._open("Vector", DatumVectorDialog)

    def datum_plane(self):
        self._open("Plane", DatumPlaneDialog)
    ...
    def _open(self, kind, dialog_type):
        ...
        dialog.reference_preview_requested.connect(
            self.ctx.parent.viewport.show_datum_reference_preview
        )
        dialog.apply_requested.connect(
            lambda values: self._commit(part.id, dialog, state, values)
        )
    ...
    def _commit(self, part_id, dialog, state, values):
        ...
        dialog.existing_names = tuple(
            item.name for item in current_part.datums if item.id != state["target_id"]
        )
