"""Own Part partition-feature dialogs, previews, validation, and commits."""
...
class PartPartitions:
    """Coordinate Part geometry-history editing and persistent previews."""
    ...
    def partition(self):
        ...
        if self.ctx.require_geometry(part):
            self._open_dialog(part)

    def edit_partition(self, feature):
        part = self.ctx.active_part()
        if part:
            self._open_dialog(part, feature)
    ...
    def _open_dialog(self, part, feature=None):
        ...
        def pick(dimension, _owner, done, finished):
            if dimension == "datum_plane":
                ...
                def selected(hit):
                    plane = (
                        self.ctx.store.project.try_resolve(hit.entity_id)
                        if hit.entity_id
                        else None
                    )
                    if isinstance(plane, DatumPlane):
                        ...
                        self.ctx.parent.viewport.message.emit(
                            "The selected datum plane no longer exists"
                        )

                return self.ctx.parent.viewport.begin_selection_session(
                    policy,
                    selected,
                    finished=finished,
                )
        ...
        dialog = PartitionDialog(
            ...
            create_datum_plane=lambda owner, done, pid=part.id: self._create_datum_plane(
                pid,
                owner,
                done,
            ),
        ...
        for selector in (
            dialog.plane_targets,
            dialog.face_targets,
            dialog.edge_parameter_targets,
            dialog.edge_vertex_targets,
            dialog.edge_vertex,
        ):
            selector.value_changed.connect(preview)
        dialog.stack.currentChanged.connect(preview)
        dialog.committed.connect(
            lambda values, pid=part.id, s=state: s.update(
                feature_id=self._apply(values, pid, s["feature_id"])
            )
        )
        dialog.finished.connect(
            lambda _code, d=dialog, prefix=preview_prefix: self._finish_dialog(
                d,
                prefix,
            )
        )
    ...
    def _apply(self, values, part_id, feature_id):
        ...
        existing = (
            candidate.geometry[existing_index]
        ...
        if cls is PartitionPlaneFeature:
            ...
            datum = next(
                (
                    item
                    for item in part.datums
                    if isinstance(item, DatumPlane) and item.id == datum_id
                ),
                None,
            )
            if datum is None:
                self.ctx.store.message.emit(
                    "Select an existing datum plane for the partition"
                )
                return feature_id
            kwargs.update(
                origin=tuple(datum.origin),
                normal=tuple(datum.normal),
            ...
            kwargs.update(
                ...
                fraction=feature_values.get("fraction", 0.5),
        ...
        if self.ctx.validate_geometry(candidate, "Partition failed"):
            self.ctx.commit_geometry_candidate(
                candidate,
                f"{'Edited' if existing else 'Created'} {replacement.name}",
            )
    ...
    def _create_datum_plane(self, part_id, owner, done):
        ...
        if part is None:
            return
        viewport = self.ctx.parent.viewport
        dialog = DatumPlaneDialog(
            next_name("Datum Plane", part.datums),
            [item.name for item in part.datums],
            part.coordinate_systems,
            owner or self.ctx.parent,
        )
        dialog.pick_requested.connect(
            lambda allowed, callback, finished: viewport.begin_datum_reference_pick(
                allowed,
                callback,
                finished,
            )
        ...
        dialog.reference_preview_requested.connect(
            viewport.show_datum_reference_preview
        )
    ...
    def rebuild_geometry(self):
        """Force a geometry rebuild without copying the generated FE mesh."""
        part = self.ctx.active_part()
        candidate = self.ctx.geometry_candidate(part) if part is not None else None
        if (
            self.ctx.require_geometry(part)
            and candidate is not None
            and self.ctx.validate_geometry(candidate, "Geometry rebuild failed")
        ):
    ...
    def suppress_feature(self):
        part = self.ctx.active_part()
        selected = self.ctx.store.selection
        feature = selected if isinstance(selected, GeometryFeature) else None
        if feature is None and part:
            feature = next(
                (
                    item
                    for item in reversed(part.geometry)
                    if not isinstance(item, ImportedStepFeature)
                ),
                None,
            )
        if part is None or feature is None:
            self.ctx.store.message.emit("Select a partition feature first")
            return
        if isinstance(feature, ImportedStepFeature):
            self.ctx.store.message.emit(
                "The source geometry feature cannot be suppressed"
            )
            return
        candidate, target = self.ctx.feature_copy(feature)
        target.suppressed = not target.suppressed
        candidate.mesh.status = "Outdated"
        if self.ctx.validate_geometry(candidate, "Feature update failed"):
            self.ctx.commit_geometry_candidate(
                candidate,
                f"{'Resumed' if not target.suppressed else 'Suppressed'} {target.name}",
            )
