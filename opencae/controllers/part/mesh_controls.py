...
from opencae.model.selection import (
    ...
    definition_from_local_labels, region_definition_error,
)
...
class PartMeshControls:
    """Legacy mesh-control facade using the canonical recipe command paths."""
    ...
    def _dialog(self, part, definition=None, control=None):
        def pick(scope, _owner, done, finished):
            ...
            policy = SelectionPolicy.create({kind}, multiple=True, requirement=RegionRequirement(RegionProjection.ELEMENTS, (dimension,), 0))
        ...
        dialog = MeshControlDialog(
            ...
            definition=definition or getattr(control, "target", None), pick_callback=pick,
            control=control, parent=self.ctx.parent,
        )
        ...
        def preview(value):
            self.ctx.parent.viewport.show_region_preview(preview_channel, value, color="#3296e6", opacity=.62, point_size=16, show_point_labels=False)
        dialog.target.set_requirement(requirement(dialog.scope.currentText()), allow_part_local=True)
        dialog.scope.currentTextChanged.connect(lambda value: dialog.target.set_requirement(requirement(value), allow_part_local=True))
        dialog.target.value_changed.connect(preview)
        def finish(_code):
            self.ctx.parent.viewport.cancel_context_pick(); self.ctx.parent.viewport.clear_region_preview(preview_channel)
        dialog.finished.connect(finish); preview(dialog.target.definition()); return dialog
    ...
    def _commit(self, part_id, values, control_id=None):
        ...
        if error:
            self.ctx.store.message.emit(error); return
        current = self.ctx.store.project.try_resolve(control_id) if control_id else None
        kwargs = {"name": values["name"], "scope": values["scope"], "topology": values["topology"], "target": target}
        replacement = create_mesh_control(values["technique"], id=current.id, **kwargs) if current else create_mesh_control(values["technique"], **kwargs)
        collection = "mesh.recipe.element_controls"
        ...
        command = CompositeCommand((mutation, UpdateFieldCommand(part_id, "mesh.lifecycle.validity", part.mesh.lifecycle.validity, MeshValidity.OUTDATED)))
    ...
    def mesh_settings(self):
        part = self.ctx.active_part()
        if part is None:
            self.ctx.store.message.emit("Create or import a part first"); return
        ...
        commands = [UpdateFieldCommand(part.id, "mesh.recipe.settings", part.mesh.settings, candidate)]
        if part.mesh.lifecycle.validity is not MeshValidity.OUTDATED:
            commands.append(UpdateFieldCommand(part.id, "mesh.lifecycle.validity", part.mesh.lifecycle.validity, MeshValidity.OUTDATED))
