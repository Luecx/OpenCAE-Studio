"""Coordinate editing and deletion of the object currently selected in the project UI."""
...
class SelectionController:
    """Dispatch edit/delete operations for the current project selection."""
    ...
    def edit_selected(self):
        """Open the dedicated editor for the selected model object when available."""
        ...
        dialog.apply()
        try:
            parent_id, attribute = entity_collection_location(
                self.store.project,
                entity.id,
            )
    ...
    def delete_selected(self):
        """Delete the selection, resolving references before destructive changes."""
        entity = self.store.selection
        if entity is None or isinstance(entity, ViewportSelection):
        ...
        uses = [
            use
            for use in project.references_to(entity.id)
            if use.source_id != entity.id
        ]
        if not uses:
            if self._confirm_direct_deletes() and QMessageBox.question(
                self.parent,
                "Delete object",
    ...
    def _confirm_direct_deletes(self) -> bool:
        """Return the workstation preference for unreferenced object deletion."""
        settings = getattr(getattr(self.parent, "context", None), "settings", None)
        reader = getattr(settings, "preference", None)
        if not callable(reader):
            return True
        return bool(reader("ui/confirm_delete", True))

    def _delete_referenced(self, entity, uses):
        """Require an explicit replacement/cascade choice for referenced entities."""
        ...
        box.setText(
            f'Cannot delete "{entity.name}" without resolving its references.'
        )
        ...
        if clicked is replace_button:
            ...
            if not candidates:
                QMessageBox.information(
                    self.parent,
                    "No replacement",
                    "No type-compatible replacement exists in the current project.",
                )
                return
    ...
    def _delete_direct(self, entity):
        """Delete one unreferenced entity directly from its owning collection."""
        try:
            parent_id, attribute = entity_collection_location(
                self.store.project,
                entity.id,
    ...
    def _replace_and_delete_command(self, old_id, replacement):
        """Build one atomic command that rewires references then deletes an entity."""
        ...
        source_ids = sorted(
            {
                use.source_id
                for use in project.references_to(old_id)
                if use.source_id != old_id
            }
        )
        for source_id in source_ids:
            ...
            commands.append(
                make_replace_command(project, parent_id, attribute, candidate)
            )
    ...
    def _cascade_delete_command(self, root_id):
        """Build one atomic command deleting a referenced entity graph."""
        ...
        roots = [
            ...
            if project.index.parent_id.get(entity_id) not in ids
        ]
        commands = []
        for entity_id in sorted(
            roots,
            key=lambda value: project.index.path.get(value, ""),
            reverse=True,
        ):
            parent_id, attribute = entity_collection_location(project, entity_id)
            commands.append(
                make_delete_command(project, parent_id, attribute, entity_id)
            )
        if not commands:
    ...
    def _replacement_label(self, entity):
        """Return a disambiguated label for replacement candidates."""
        path = self.store.project.index.path.get(entity.id, "")
        return f"{entity.name} — {path}"

    def _after_delete(self, entity):
        """Clear selection and invalidate geometry/display state after deletion."""
        ...
        if self.part_controller:
            ...
            if part:
                self.part_controller.service.invalidate(
                    part.id,
                    mesh_only=not isinstance(entity, GeometryFeature),
                )
