...
class NodeOccurrence:
    """Solver/export boundary identity for one node occurrence."""
    owner_id: str
    node_id: int
    instance_id: str = ""
...
class ResolvedRegion:
    """Flattened solver-facing projection; the persistent model remains object-based."""
    ...
    def count(self, projection: RegionProjection) -> int:
        ...
        if projection == RegionProjection.ELEMENTS:
...
class RegionResolver:
    ...
    def _resolve_definition(
        self, definition, requirement, result, stack, inherited_instance, allow_part_local
    ):
        for index, item in enumerate(definition.items):
            try:
                ...
                result.diagnostics.append(
                    RegionDiagnostic("resolution_error", str(exc), index)
                )

    def _resolve_operand(
        self, operand, requirement, result, stack, inherited_instance, allow_part_local
    ):
        ...
        if isinstance(operand, NamedRegionOperand):
            ...
            if region is None or self.project.try_resolve(region) is not region:
                result.diagnostics.append(
                    RegionDiagnostic("missing_region", "Region no longer exists")
                )
                return
            if region.id in stack:
                result.diagnostics.append(
                    RegionDiagnostic("region_cycle", f"Region cycle involving '{region.name}'")
                )
                return
            instance = _id(operand.instance) or inherited_instance
            self._resolve_definition(
                region.definition,
                requirement,
                result,
        ...
        if isinstance(operand, ReferencePointOperand):
            ...
            if point is None or self.project.try_resolve(point) is not point:
                result.diagnostics.append(
                    RegionDiagnostic("missing_reference_point", "Reference point no longer exists")
                )
                return
            instance_id = _id(operand.instance) or inherited_instance or ""
        ...
        if part is None:
            result.diagnostics.append(
                RegionDiagnostic("missing_owner", "Selection owner no longer exists")
            )
            return
        ...
        if selected_revision and revision and selected_revision != revision:
            result.diagnostics.append(
                RegionDiagnostic(
                    "stale_mesh_selection",
                    f"Mesh selection in '{part.name}' belongs to an older mesh revision",
                )
            )
            return
        ...
        if isinstance(operand, GeometryOperand):
            ...
            if operand.dimension not in requirement.allowed_dimensions:
                result.diagnostics.append(
                    RegionDiagnostic(
                        "invalid_dimension", f"{label} is not allowed for this target"
                    )
                )
                return
            if requirement.projection in {
                RegionProjection.NODES,
                RegionProjection.SINGLE_CONTROL_NODE,
            }:
                result.nodes.update(
                    NodeOccurrence(part.id, int(node), instance_id)
                    for node in part.mesh.entity_nodes.get(label, ())
                )
            elif requirement.projection == RegionProjection.ELEMENTS:
                result.elements.update(
                    ElementOccurrence(part.id, int(element), instance_id)
                    for element in part.mesh.entity_elements.get(label, ())
                )
        ...
        if isinstance(operand, MeshNodeOperand):
            node = operand.node
            if node is None or _canonical_node(part, node) is not node:
                result.diagnostics.append(
                    RegionDiagnostic("missing_node", "Selected Node no longer exists in the Part")
                )
                return
            result.nodes.add(NodeOccurrence(part.id, node.id, instance_id))
            return

        if isinstance(operand, MeshElementOperand):
            element = operand.element
            element_id = getattr(element, "id", -1)
            connectivity = _element_connectivity(part, element_id)
            if connectivity is None:
                result.diagnostics.append(
                    RegionDiagnostic("missing_element", "Selected Element no longer exists in the Part")
                )
                return
            if requirement.projection in {
                RegionProjection.NODES,
                ...
                result.elements.add(ElementOccurrence(part.id, element_id, instance_id))
            return

        if isinstance(operand, MeshFacetOperand):
            element = operand.element
            element_id = getattr(element, "id", -1)
            block = _element_block(part, element_id)
            if block is None:
                result.diagnostics.append(
                    RegionDiagnostic("missing_element", "Selected Element no longer exists in the Part")
                )
            ...
            allowed = (
                ...
                else {side for side, _ in element_side_indices(block.definition.topology)}
            )
            if operand.local_face not in allowed:
                result.diagnostics.append(
                    RegionDiagnostic(
                        "invalid_element_face",
                        f"Element {element_id} has no local face '{operand.local_face}'",
                    )
                )
                return
            result.facets.add(
                FacetOccurrence(part.id, element_id, operand.local_face, instance_id)
            )
            if requirement.projection in {
                RegionProjection.NODES,
                RegionProjection.SINGLE_CONTROL_NODE,
            }:
                connectivity = _facet_connectivity(part, element_id, operand.local_face)
                result.nodes.update(
                    NodeOccurrence(part.id, int(node), instance_id)
                    for node in connectivity
                )
    ...
    def _resolve_whole_model(self, operand, requirement, result, inherited_instance):
        instance = operand.instance
        explicit_instance = _id(instance) or inherited_instance or ""
        if explicit_instance:
            if instance is None:
                instance = self.project.try_resolve(explicit_instance)
            part = getattr(instance, "part", None) if instance else None
            if part is None:
                result.diagnostics.append(
                    RegionDiagnostic("missing_owner", "Whole-model occurrence no longer exists")
                )
                return
        ...
        if isinstance(owner, Instance):
            if owner.part is not None:
                self._add_entire_part(owner.part, owner.id, result)
            return
        ...
        if active:
            for occurrence in active:
                if occurrence.part is not None:
                    self._add_entire_part(occurrence.part, occurrence.id, result)
            return
    ...
    def _add_entire_part(part, instance_id, result):
        result.nodes.update(
            NodeOccurrence(part.id, node.id, instance_id) for node in part.mesh.nodes
        )
        for element in part.mesh.iter_elements():
            result.elements.add(ElementOccurrence(part.id, element.id, instance_id))

    def _owner(self, operand, inherited_instance):
        instance = getattr(operand, "instance", None)
        instance_id = _id(instance) or inherited_instance or ""
    ...
    def _derive(self, requirement, result):
        if requirement.projection in {
            ...
            for element in tuple(result.elements):
                ...
                if connectivity:
                    result.nodes.update(
                        NodeOccurrence(element.owner_id, int(node), element.instance_id)
                        for node in connectivity
                    )
        elif requirement.projection == RegionProjection.FACETS:
            groups = {}
            for element in result.elements:
                groups.setdefault((element.owner_id, element.instance_id), set()).add(
                    element.element_id
                )
            for (owner_id, instance_id), element_ids in groups.items():
                part = self.project.try_resolve(owner_id)
                if part:
                    result.facets.update(
                        _boundary_facets(part, element_ids, instance_id)
                    )
    ...
    def _validate(self, requirement, result):
        ...
        if count < requirement.min_count:
            result.diagnostics.append(
                RegionDiagnostic(
                    "too_few_members",
                    f"Target resolves to {count} member(s), at least {requirement.min_count} required",
                )
            )
        if requirement.max_count is not None and count > requirement.max_count:
            result.diagnostics.append(
                RegionDiagnostic(
                    "too_many_members",
                    f"Target resolves to {count} member(s), at most {requirement.max_count} allowed",
                )
            )
        if requirement.require_unique_occurrence:
            ...
            if len(instances) > 1:
                result.diagnostics.append(
                    RegionDiagnostic(
                        "multiple_occurrences", "Target spans multiple assembly occurrences"
                    )
                )
...
def _canonical_node(part, node):
    try:
        return part.mesh.node(node.id)
    except (AttributeError, KeyError, TypeError, ValueError):
        return None


def _geometry_label(dimension, tag):
    return f"{('Vertex', 'Edge', 'Face', 'Cell')[int(dimension)]}-{int(tag)}"
...
def _element_connectivity(part, element_id):
    if part is None:
        return None
    for block in part.mesh.element_blocks:
        for current, connectivity in zip(block.ids, block.connectivity, strict=True):
            if int(current) == int(element_id):
                return tuple(int(value) for value in connectivity)
    return None
...
def _facet_connectivity(part, element_id, local_face):
    ...
    for block in part.mesh.element_blocks:
        for current, connectivity in zip(block.ids, block.connectivity, strict=True):
            ...
            if block.definition.category in {"Shell Elements", "2D Elements"}:
                return (
                    tuple(int(value) for value in connectivity)
                    if local_face in {"SPOS", "SNEG"}
                    else ()
                )
            indices = dict(element_side_indices(block.definition.topology)).get(
                str(local_face), ()
            )
            return tuple(
...
def _boundary_facets(part, element_ids, instance_id):
    ...
    for block in part.mesh.element_blocks:
        ...
        for element_id, connectivity in zip(block.ids, block.connectivity, strict=True):
            element_id = int(element_id)
            if element_id not in selected:
                continue
            if category in {"Shell Elements", "2D Elements"}:
                shell_facets.add(
                    FacetOccurrence(part.id, element_id, "SPOS", instance_id)
                )
                continue
