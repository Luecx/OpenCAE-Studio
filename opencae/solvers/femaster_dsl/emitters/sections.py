...
def write_section(section, elset, orientation, writer, context):
    ...
    callback = {"Solid": _write_solid, "Truss": _write_truss, "Beam": _write_beam, "Shell": _write_shell}.get(section.section_type)
    if callback:
...
def _required_name(ref, label, section, context):
    ...
    if entity is None:
        legacy = ref.legacy_name if ref else ""
        if legacy:
            return legacy
...
def _write_solid(section, elset, orientation, writer, context):
    command(writer, "SOLIDSECTION", ELSET=elset, MATERIAL=_material(section, context), ORIENTATION=orientation)


def _write_truss(section, elset, orientation, writer, context):
    command(writer, "TRUSSSECTION", ELSET=elset, MATERIAL=_material(section, context), AREA=section.area)


def _write_beam(section, elset, orientation, writer, context):
    command(writer, "BEAMSECTION", [tuple(getattr(section, "direction", (0.0, 1.0, 0.0)))], ELSET=elset, MATERIAL=_material(section, context), PROFILE=_profile(section, context))
...
def _write_shell(section, elset, orientation, writer, context):
    if section.shell_definition.startswith("ABD"):
        ...
        command(writer, "SHELLSECTION", [*section.abd_matrix, shear_row], ELSET=elset, TYPE="ABD", THICKNESS=section.thickness or None, ORIENTATION=orientation)
        return
    command(writer, "SHELLSECTION", [(section.thickness,)], ELSET=elset, TYPE="INTEGRATED", MATERIAL=_material(section, context), ORIENTATION=orientation)
