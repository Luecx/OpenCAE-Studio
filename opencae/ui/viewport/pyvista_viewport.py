...
class PyVistaViewport(QWidget):
    ...
    def _perform_refresh(self):
        ...
        if self._pending_element_control_preview is not None:
            ...
            self.element_control_overlay.show(self.plotter, self.scene, selected, propagated)
            self.plotter.render()
    ...
    def set_selection_mode(self, mode):
        ...
        # Configure actor visibility before rendering. Previously clear() drew
        # the old state and configure() changed point visibility without a new
    ...
    def _restore_reference_point_preview(self, render=True):
        ...
        self.scene.reference_overlay.show_preview(self.plotter, name, position)
        if render:
            self.plotter.render()
    ...
    def toggle_mesh(self):
        self.set_display_mode("mesh" if self.display_mode == "geometry" else "geometry")
    ...
    def show_solution(self, result, field=None, options=None):
        ...
        self.result_selection_panel.set_selection((options or {}).get("selection", {}))
        self.canvas._position_overlays()
    ...
    def is_showing_result(self, result):
        return bool(
            ...
            and getattr(self._active_result, "id", None) == getattr(result, "id", None)
        )
