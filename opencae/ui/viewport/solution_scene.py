...
def show_result(scene, result, field=None, options=None):
    ...
    try:
        scene.result_actor, scene.result_grid, scene.result_mesh_actor, scene.result_boundary_actor, scene.result_undeformed_actor = add_result(scene.owner.plotter, result, field, options)
