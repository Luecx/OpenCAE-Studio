...
class ResultQueryState:
    def __init__(self,owner):self.owner=owner; self.mode=""; self.field=None; self._marker="result-query-marker"; self._edges="result-query-edges"
    ...
    def _remove_marker(self):
        for name in (self._marker,self._edges):
            try:self.owner.plotter.remove_actor(name,reset_camera=False,render=False)
            except Exception:pass
    def _picked(self,point):
        ...
        else:index,result=element_values(grid,point,self.field); marker=grid.get_cell(index).center; title="Element Query"+suffix; self._highlight_element(grid,index)
    ...
    def _highlight_element(self,grid,index):
        try:
            self.owner.plotter.remove_actor(self._edges,reset_camera=False,render=False)
        except Exception:
            pass
        try:
            edges=grid.extract_cells([int(index)]).extract_all_edges()
            self.owner.plotter.add_mesh(edges,color="#f2b84b",line_width=4.0,lighting=False,name=self._edges,pickable=False,render=False)
        except Exception:
            pass
