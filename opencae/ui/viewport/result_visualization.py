...
def add_result(plotter, result, field=None, options=None):
    ...
    actor = plotter.add_mesh(grid, scalars=scalar, clim=clim, cmap="turbo", n_colors=18, show_edges=show_edges,
    ...
    return actor, grid, boundary, undeformed
...
def _deformed(grid, options):
    ...
    keys = next((group for group in candidates if all(key in grid.point_data for key in group)), None)
