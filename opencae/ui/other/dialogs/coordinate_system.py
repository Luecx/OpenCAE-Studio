...
from opencae.ui.components import ControlXYZPicker
...
class CoordinateSystemDialog(QDialog):
    ...
    def __init__(self, default_name="CSYS-1", existing_names=(), parent=None, units=None):
        ...
        root.addWidget(LabelSection("Coordinate System Definition"))
        length_unit = units.symbol("length") if units is not None else ""
        self.origin = ControlXYZPicker(
            allowed=_POINT_KINDS,
            value_kind="point",
            suffix=length_unit,
        )
        self.axis_1 = ControlXYZPicker(
            (1.0, 0.0, 0.0),
            allowed=_DIRECTION_KINDS,
            value_kind="direction",
            suffix=length_unit,
        )
        self.axis_2 = ControlXYZPicker(
            (0.0, 1.0, 0.0),
            allowed=_DIRECTION_KINDS,
            value_kind="direction",
            suffix=length_unit,
        ...
        root.addWidget(FormField("Origin", self.origin))
        self.axis_1_field = FormField("X direction", self.axis_1)
        self.axis_2_field = FormField("Y direction", self.axis_2)
