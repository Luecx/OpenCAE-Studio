...
from opencae.ui.components import AmplitudeCurvePreview
...
class AmplitudeDialog(ApplyDialog):
    ...
    def __init__(
        ...
        root.addWidget(LabelSection("Amplitude Definition"))
        ...
        content_layout.addWidget(FormField("Definition", self.mode), 0, 0)
        content_layout.addWidget(FormField("Time basis", self.time_basis), 0, 1)
    ...
    def _tabular_page(self, amplitude):
        ...
        layout.addWidget(FormField("Interpolation", self.interpolation))
    ...
    def _function_page(self, amplitude):
        ...
        layout.addWidget(FormField("Function", self.function_type))
        ...
        layout.addWidget(
            FieldRow(
                FormField("Start time", self.sample_start),
                FormField("End time", self.sample_end),
                FormField("Intervals", self.sample_intervals),
            )
        )
        layout.addStretch(1)
    ...
    def _rebuild_parameters(self, _index=None):
        ...
        for key, value in defaults.items():
            ...
            fields.append(FormField(_parameter_label(key), editor))
        if len(fields) <= 2:
            self.parameter_layout.addWidget(FieldRow(*fields))
        else:
            for start in range(0, len(fields), 2):
                self.parameter_layout.addWidget(FieldRow(*fields[start:start + 2]))
