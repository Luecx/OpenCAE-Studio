...
from opencae.ui.components import MatrixEditor
...
class SectionDialog(ApplyDialog):
    ...
    def __init__(
        ...
        root.addWidget(FormField("Section type", self.kind))
    ...
    def _reference_page(cls, text, values, current, callback):
        ...
        layout.addWidget(FormField(text, selector))
        layout.addStretch(1)
    ...
    def _shell_page(self, materials, current, callback):
        ...
        layout.addWidget(FormField("Shell formulation", self.shell_definition))
        ...
        integrated_layout.addWidget(FormField("Material", self.shell_material))
        integrated_layout.addWidget(
            FieldRow(
                FormField("Thickness", self.shell_thickness),
                FormField("Integration points", self.shell_points),
            )
        )
    ...
    def _beam_page(self, materials, profiles, material, profile, cm, cp):
        ...
        layout.addWidget(
            FieldRow(
                FormField("Material", mat),
                FormField("Profile", prof),
            )
        )
        n1 = ControlVector3(
        ...
        layout.addWidget(FormField("n1 direction", n1))
        layout.addStretch(1)
    ...
    def _truss_page(self, materials, material, callback):
        ...
        layout.addWidget(
            FieldRow(
                FormField("Material", mat),
                FormField("Cross-sectional area", area),
            )
        )
        layout.addStretch(1)
