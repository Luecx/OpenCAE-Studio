...
from opencae.ui.components.controls import ControlReadOnlyValue, ControlVector3
...
class MeshNodeBulkDialog(QDialog):
    ...
    def __init__(self, nodes, parent=None):
        ...
        root.addWidget(LabelSection("Selected Nodes"))
        ...
        root.addWidget(LabelSection("Bulk Translation"))
        self.translation = ControlVector3((0.0, 0.0, 0.0), labels=("dX", "dY", "dZ"))
        root.addWidget(FormField("Offset added to every row", self.translation))
        root.addWidget(
            FormField(
                "Selection",
                ControlReadOnlyValue(f"{len(self.nodes)} node(s)"),
            )
        )
        root.addStretch(1)
...
class MeshElementBulkDialog(QDialog):
    ...
    def __init__(self, elements, *, available_node_ids=(), parent=None):
        ...
        root = dialog_layout(self)
        root.addWidget(LabelSection("Bulk Type Change"))
        ...
        apply_primary_input_geometry(self.element_type)
        root.addWidget(FormField("Apply type to all selected elements", self.element_type))

        root.addWidget(LabelSection("Selected Elements"))
        ...
        root.addWidget(
            FormField(
                "Selection",
                ControlReadOnlyValue(f"{len(self.elements)} element(s)"),
            )
        )
        root.addStretch(1)
        buttons = dialog_buttons()
