...
class StepDialog(QDialog):
    ...
    def __init__(self, step, loads, supports, parent=None, existing_names=()):
        ...
        if step.step_type in {StepType.EIGENFREQUENCY, StepType.LINEAR_BUCKLING}:
            root.addWidget(
                FieldRow(
                    ...
                    FormField("Number of modes", self.modes),
                )
            )
        else:
        ...
        if step.uses_loads:
            ...
            root.addWidget(
                FieldRow(
                    support_field,
                    FormField("Active loads", self.loads),
                )
            )
        else:
    ...
    def _build_nonlinear_controls(self):
        ...
        increment_layout.addWidget(
            FieldRow(
                FormField("Control method", self.nonlinear_control),
                FormField("Maximum increments", self.max_increments),
                FormField("Iterations / increment", self.max_iterations),
            )
        )
        ...
        convergence_layout.addWidget(
            FieldRow(
                FormField("Convergence tolerance", self.tolerance),
                FormField("Growth factor", self.growth_factor),
                FormField("Cutback factor", self.cutback_factor),
            )
        )
        ...
        convergence_layout.addWidget(
            FieldRow(
                FormField("Fast convergence", self.fast_iterations),
                FormField("Slow convergence", self.slow_iterations),
                FormField("Maximum cutbacks", self.maximum_cutbacks),
            )
        )
        ...
        convergence_layout.addWidget(FormField("Regularization scale", self.regularization_alpha))
    ...
    def _build_load_control_page(self, settings):
        ...
        layout.addWidget(
            FieldRow(
                FormField("Total load period", self.time_period),
                FormField("Initial increment", self.initial_increment),
            )
        )
        layout.addWidget(
            FieldRow(
                FormField("Minimum increment", self.minimum_increment),
                FormField("Maximum increment", self.maximum_increment),
            )
        )
        return page
    ...
    def _build_path_control_page(self, settings):
        ...
        layout.addWidget(
            FieldRow(
                FormField("Estimated total arc length", self.total_arc_length),
                FormField("Initial arc length", self.initial_arc_length),
            )
        )
        layout.addWidget(
            FieldRow(
                FormField("Minimum arc length", self.minimum_arc_length),
                FormField("Maximum arc length", self.maximum_arc_length),
                FormField("Path load weighting (ψ)", self.arc_length_psi),
            )
        )
        return page
