...
from opencae.ui.components.controls import ControlNumericUnit, ControlReferenceSelector
from opencae.ui.components import ComponentsWidget
...
class LoadDialog(BaseLoadDialog):
    ...
    def __init__(
        ...
        if load_type == "Concentrated Load":
            ...
            self.root.addWidget(LabelSection("Load Components"))
            ...
            self.root.addWidget(LabelSection("Inertia Definition"))
            self.root.addWidget(
                FieldRow(
                    FormField("Center", self.inertia[0]),
                    FormField("Center acceleration", self.inertia[1]),
                )
            )
            self.root.addWidget(
                FieldRow(
                    FormField("Angular velocity", self.inertia[2]),
                    FormField("Angular acceleration", self.inertia[3]),
                )
            )
        ...
        if load_type in _AMPLITUDE_LOAD_TYPES:
            ...
            self.root.addWidget(LabelSection("Amplitude"))
            self.amplitude = ControlReferenceSelector(
                (("None", None), *tuple(amplitudes)),
                current_amplitude,
            )
            self.root.addWidget(FormField("Amplitude", self.amplitude))
