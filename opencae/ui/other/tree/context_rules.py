...
def available(action_id, store, kind):
    ...
    if action_id == A.EDIT_NODE:
        return bool(part and part.mesh.node_count)
    if action_id in {A.DELETE_NODE, A.EDIT_ELEMENT, A.DELETE_ELEMENT}:
    ...
    if action_id in {
        ...
        A.CONSTRAINT_MPC, A.FIXED, A.DISPLACEMENT, A.SYMMETRY, A.CLOAD,
        A.DLOAD, A.PRESSURE, A.VLOAD, A.INERTIA_LOAD, A.TEMPERATURE,
