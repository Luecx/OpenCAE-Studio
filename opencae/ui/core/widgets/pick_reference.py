...
class PickReference(QWidget):
    ...
    def __init__(self, allowed, parent=None):
        ...
        pick = QPushButton("Pick"); clear = QPushButton("×"); clear.setFixedWidth(30)
        pick.clicked.connect(lambda: self.pick_requested.emit(self.allowed, self.set_reference)); clear.clicked.connect(lambda: self.set_reference(None))
        layout.addWidget(self.text, 1); layout.addWidget(pick); layout.addWidget(clear)
