...
class IconKind(Enum):
    ...
    SKETCH_ARC = auto(); SKETCH_ARC_CENTER = auto(); SKETCH_ARC_3POINT = auto()
    SKETCH_ELLIPSE = auto(); SKETCH_SPLINE = auto(); SKETCH_SLOT = auto()
    SKETCH_PRIMITIVES_MORE = auto(); SKETCH_CONSTRUCTION = auto(); SKETCH_GRID = auto(); SKETCH_SNAP = auto()

    SKETCH_CONSTRAINT = auto(); SKETCH_CONSTRAINT_COINCIDENT = auto()
    SKETCH_CONSTRAINT_HORIZONTAL = auto(); SKETCH_CONSTRAINT_VERTICAL = auto()
    SKETCH_CONSTRAINT_PARALLEL = auto(); SKETCH_CONSTRAINT_PERPENDICULAR = auto()
    SKETCH_CONSTRAINT_TANGENT = auto(); SKETCH_CONSTRAINT_EQUAL = auto()
    SKETCH_CONSTRAINT_CONCENTRIC = auto(); SKETCH_CONSTRAINT_MIDPOINT = auto()
    SKETCH_CONSTRAINT_COLLINEAR = auto(); SKETCH_CONSTRAINT_POINT_ON = auto()
    SKETCH_CONSTRAINT_SYMMETRY = auto(); SKETCH_CONSTRAINT_FIXED = auto()
    SKETCH_CONSTRAINT_MORE = auto()

    SKETCH_DIMENSION = auto(); SKETCH_DIMENSION_DISTANCE = auto()
    SKETCH_DIMENSION_HORIZONTAL = auto(); SKETCH_DIMENSION_VERTICAL = auto()
    SKETCH_DIMENSION_ANGLE = auto(); SKETCH_DIMENSION_RADIUS = auto()
    SKETCH_DIMENSION_DIAMETER = auto()
