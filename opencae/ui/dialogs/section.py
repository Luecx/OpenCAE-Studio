...
from PyQt6.QtWidgets import QDoubleSpinBox, QGroupBox, QLineEdit, QMessageBox, QSpinBox, QStackedWidget, QVBoxLayout, QWidget
...
class SectionDialog(ApplyDialog):
    def __init__(self, materials=(), profiles=(), create_material=None, create_profile=None, section=None, existing_names=(), parent=None, initial_type=None, default_name="Section-1", units=None):
        ...
        self.solid_material = self._reference_page("Material", materials, material_current, create_material)
        ...
        self.beam_page, self.beam_material, self.beam_profile = self._beam_page(materials, profiles, material_current, profile_current, create_material, create_profile)
        self.stack.addWidget(self.beam_page)
        self.truss_page, self.truss_material, self.truss_area = self._truss_page(materials, material_current, create_material)
    ...
    @staticmethod
    def _reference_page(text, values, current, callback):
        page = QWidget()
    ...
    def _shell_page(self, materials, current, callback):
        ...
        self.shell_definition.setCurrentText(getattr(self.section, "shell_definition", SHELL_TYPES[0]))
        ...
        self.shell_thickness = QDoubleSpinBox()
        self.shell_thickness.setRange(1e-12, 1e12)
        self.shell_thickness.setDecimals(6)
        self.shell_thickness.setSuffix(self.units.suffix("length") if self.units is not None else "")
        self.shell_thickness.setValue(getattr(self.section, "thickness", 1.0))
    ...
    def _truss_page(self, materials, material, callback):
        ...
        area = QDoubleSpinBox()
        area.setRange(1e-12, 1e30)
        area.setDecimals(6)
        area.setSuffix(self.units.suffix("area") if self.units is not None else "")
        area.setValue(getattr(self.section, "area", 1.0))
        form.addRow("Material", mat)
    ...
    def validate(self):
        ...
        if not name:
            ...
            return False
        if name.casefold() in self.existing_names and (self.section is None or name.casefold() != self.section.name.casefold()):
            QMessageBox.warning(self, "Duplicate name", f"A section named '{name}' already exists.")
            return False
        if kind in {"Solid", "Beam", "Truss"} and not values.get("material_ref"):
            QMessageBox.warning(self, "Missing material", "Create or select a material first.")
            return False
        if kind == "Beam" and not values.get("profile_ref"):
            QMessageBox.warning(self, "Missing profile", "Create or select a beam profile first.")
            return False
        if kind == "Shell" and values["shell_definition"] == SHELL_TYPES[0] and not values.get("material_ref"):
            QMessageBox.warning(self, "Missing material", "Create or select a material first.")
            return False
        return True
    ...
    def values(self):
        ...
        if kind == "Solid":
            result["material_ref"] = self._ref(self.solid_material[1].currentValue(), "Material")
        elif kind == "Beam":
            result.update(material_ref=self._ref(self.beam_material.currentValue(), "Material"), profile_ref=self._ref(self.beam_profile.currentValue(), "Profile"))
        elif kind == "Truss":
            result.update(material_ref=self._ref(self.truss_material.currentValue(), "Material"), area=self.truss_area.value())
        else:
            result["shell_definition"] = self.shell_definition.currentText()
            result["material_ref"] = self._ref(self.shell_material.currentValue(), "Material") if self.shell_definition.currentIndex() == 0 else None
