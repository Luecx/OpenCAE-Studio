...
def render_u_profile(painter: QPainter, area: QRectF, values: dict) -> None:
    ...
    base = min(base_thickness * outer.height() / leg_height, outer.height() * 0.8)
    leg = min(leg_thickness * outer.width() / overall_width, outer.width() * 0.4)
    ...
    draw_internal_dimension(
        ...
        dimension_symbol("web_thickness"), QPointF(14.0, 0.0),
    )
    draw_internal_dimension(
        painter, QPointF(outer.left(), outer.top() + 12.0),
        QPointF(x_left, outer.top() + 12.0),
        dimension_symbol("flange_thickness"), QPointF(0.0, -11.0),
    )
