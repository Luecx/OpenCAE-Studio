...
class LoadDialog(BaseLoadDialog):
    def __init__(self,load_type,project,regions=(),coordinate_systems=(),fields=(),create_region=None,pick_region=None,parent=None,default_name="",existing_names=(),load=None,target_validator=None,target_requirement=None,units=None):
        ...
        suffix=lambda quantity: units.suffix(quantity) if units is not None else ""
        if load_type=="Concentrated Load":
            self.components=ComponentsWidget(("Fx","Fy","Fz","Mx","My","Mz"),getattr(load,"components",[0.0]*6),suffixes=(suffix("force"),)*3+(suffix("moment"),)*3);self.root.addWidget(self.components)
        ...
        elif load_type=="Surface Traction":self.components=ComponentsWidget(("Fx","Fy","Fz"),getattr(load,"components",[0.0]*3),suffixes=suffix("pressure"));self.root.addWidget(self.components)
        elif load_type=="Volume Load":self.components=ComponentsWidget(("Fx","Fy","Fz"),getattr(load,"components",[0.0]*3),suffixes=suffix("volume_load"));self.root.addWidget(self.components)
        elif load_type=="Pressure":self.scalar=self._number(getattr(load,"pressure",1.0),suffix("pressure"));self.form.addRow("Pressure",self.scalar)
        elif load_type=="Temperature":
            self.scalar=self._number(getattr(load,"reference_temperature",0.0),suffix("temperature"));self.form.addRow("Reference temperature",self.scalar);current=load.temperature_field_ref.entity_id if load and getattr(load,"temperature_field_ref",None) else (fields[0].id if fields else "");self.temperature_field=ReferenceSelector(fields,current);self.form.addRow("Temperature field",self.temperature_field)
        elif load_type=="Inertia Load":
            values=(getattr(load,"center",(0,0,0)),getattr(load,"center_acceleration",(0,0,0)),getattr(load,"angular_velocity",(0,0,0)),getattr(load,"angular_acceleration",(0,0,0)));quantities=("length","acceleration","frequency","angular_acceleration");self.inertia=[ComponentsWidget(("X","Y","Z"),value,suffixes=suffix(quantity)) for value,quantity in zip(values,quantities)]
    ...
    @staticmethod
    def _number(value,suffix=""):widget=QDoubleSpinBox();widget.setDecimals(12);widget.setRange(-1e300,1e300);widget.setSuffix(suffix);widget.setValue(value);return widget
