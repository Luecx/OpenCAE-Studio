...
from opencae.ui.templates import FieldStack, LabelRole, SectionHeading, apply_primary_control_height, label
...
class RunAnalysisDialog(QDialog):
    ...
    def __init__(self, analysis, solver_adapters, settings, parent=None):
        ...
        self._refresh_profiles(reset=False, requested=getattr(analysis, "deck_profile", ""))
    ...
    def _adapter(self):
        return self._adapters[str(self.solver.currentData() or self.solver.currentText())]

    def _refresh_profiles(self, *, reset: bool, requested: str = "") -> None:
        """Refresh profile choices and reset to the solver's built-in when requested."""
        adapter = self._adapter()
        selected = default_profile_name(adapter) if reset else normalized_profile_name(
            self._settings, adapter, requested
        )
    ...
    def values(self) -> tuple[str, str]:
        """Return the selected solver and normalized compatible profile name."""
        adapter = self._adapter()
        return (
