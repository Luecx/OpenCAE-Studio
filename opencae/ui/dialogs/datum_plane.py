...
class DatumPlaneDialog(DatumDialogBase):
    ...
    def __init__(self, default_name, existing_names=(), coordinate_systems=(), parent=None):
        ...
        self.reference = references("face","datum_plane"); self.offset = number(); self.add_page(page((("Face / plane",self.reference),("Offset",self.offset))))
        self.csys = csys_choice(coordinate_systems); self.principal = choice(("XY","YZ","ZX")); self.principal_offset = number(); self.add_page(page((("Coordinate system",self.csys),("Plane",self.principal),("Offset",self.principal_offset))))
