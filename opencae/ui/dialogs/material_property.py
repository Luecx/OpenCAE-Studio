...
class MaterialPropertyDialog(QDialog):
    def __init__(self, behavior=None, parent=None, category=None, units=None):
        ...
        form.addRow("Definition", self.kind)
        root.addLayout(form)
        ...
        for kind in self.types:
            self._add_page(kind)
        self.kind.currentTextChanged.connect(lambda text: self.stack.setCurrentIndex(self.types.index(text)))
    ...
    def _add_page(self, kind):
        ...
        editors = []
        for label, default, quantity in _PROPERTY_SPECS[kind]:
    ...
    def behavior_value(self):
        ...
        return {
            "Isotropic elasticity": lambda: IsotropicElasticity(youngs_modulus=values[0], poisson_ratio=values[1]),
            "Neo-Hooke": lambda: NeoHookeElasticity(c10=values[0], d1=values[1]),
            "Constant density": lambda: DensityBehavior(value=values[0]),
            "Isotropic expansion": lambda: IsotropicThermalExpansion(coefficient=values[0], reference_temperature=values[1]),
            "Bilinear isotropic hardening": lambda: IsotropicPlasticity(yield_stress=values[0], tangent_modulus=values[1]),
        }[kind]()
