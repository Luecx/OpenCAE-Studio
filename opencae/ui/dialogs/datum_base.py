...
from PyQt6.QtWidgets import QComboBox, QDialog, QDialogButtonBox, QFormLayout, QLabel, QLineEdit, QMessageBox, QStackedWidget, QVBoxLayout
...
class DatumDialogBase(QDialog):
    ...
    def __init__(self, title, methods, default_name, existing_names, parent=None):
        ...
        self.setAttribute(Qt.WidgetAttribute.WA_DeleteOnClose, True); self.setMinimumWidth(560)
        ...
        form = QFormLayout(); self.name = QLineEdit(default_name); self.method = QComboBox(); self.method.addItems(methods)
        form.addRow("Name", self.name); form.addRow("Method", self.method); layout.addLayout(form)
    ...
    def add_page(self, page):
        ...
        for child in page.findChildren(QComboBox): child.currentIndexChanged.connect(self.emit_preview)
    ...
    def _apply(self):
        ...
        if not name: QMessageBox.warning(self, "Missing name", "Enter a datum name."); return
        if not is_unique(name, self.existing_names): QMessageBox.warning(self, "Duplicate name", f"A datum named '{name}' already exists."); return
        try: values = self.values()
        except Exception as exc: QMessageBox.warning(self, "Incomplete definition", str(exc)); return
