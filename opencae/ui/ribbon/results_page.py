...
class ResultsPage(QWidget):
    ...
    def _build(self):
        ...
        layout.addWidget(ResultRibbonGroup(
            "VISUALS",
            (self.mesh_lines, self.boundary_lines, self.undeformed, self.deform, self.section),
        ))
        ...
        layout.addWidget(ResultRibbonGroup("QUERY", (self.query_nodes, self.query_elements)))
        layout.addStretch(1)

        for button in (self.mesh_lines, self.boundary_lines, self.undeformed):
        ...
        self.choose.navigation_changed.connect(self._set_frame_navigation)
        self.previous_frame.clicked.connect(self.choose.select_previous_frame)
        self.next_frame.clicked.connect(self.choose.select_next_frame)
        self._wire_queries()
    ...
    def set_solution(self, result, field=None):
        ...
        self.result = result
        fields = self.loader.fields(result.source_file) if result and result.source_file else []
    ...
    def _set_frame_navigation(self, has_previous, has_next):
    ...
    def _field_changed(self):
        ...
        if self.result and field:
            self.range.set_data_range(*self.loader.scalar_range(self.result.source_file, field))
        self._emit()
    ...
    def _save_results(self):
        ...
        target, _ = QFileDialog.getSaveFileName(self, "Save Results", source.name, "FRD results (*.frd)")
        if not target:
            return
        destination = Path(target)
        destination = destination if destination.suffix else destination.with_suffix(".frd")
        try:
