...
from PyQt6.QtWidgets import QHBoxLayout, QLineEdit, QStackedWidget, QTabBar, QToolButton, QVBoxLayout, QWidget
...
class ProjectPanel(QWidget):
    ...
    def __init__(self, store, actions, visibility=None, parent=None):
        ...
        self.tabs.currentChanged.connect(self._tab_changed); layout.addWidget(self.tabs)
        toolbar = QWidget(); toolbar.setStyleSheet(f"background:{PALETTE['panel']}; border-bottom:1px solid {PALETTE['border']};")
        row = QHBoxLayout(toolbar); row.setContentsMargins(7, 6, 5, 6); row.setSpacing(4)
        ...
        self.stack = QStackedWidget(); self.tree = ProjectTree(store, actions, visibility=visibility); self.solution_tree = SolutionTree(store)
    ...
    def _collapse(self): self.stack.currentWidget().collapseAll()

    @staticmethod
    def _small_button(text):
        button = QToolButton(); button.setText(text); button.setCursor(Qt.CursorShape.PointingHandCursor); button.setFixedSize(28, 28); return button
