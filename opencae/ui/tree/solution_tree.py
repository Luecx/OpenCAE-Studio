...
class SolutionTree(QTreeView):
    ...
    def rebuild(self, *_):
        ...
        for result in self.store.project.results:
            ...
            result_item = self._item(result.name, result, None, IconKind.RESULTS)
            root.appendRow(result_item)
            for step_index, step_id in enumerate(step_ids(result.fields)):
                step = self._item(step_label(result, step_id, step_index), result, None, IconKind.RESULT_STEP)
                result_item.appendRow(step)
                for frame_id, value in frame_keys(result.fields, step_id):
                    frame = self._item(frame_label(frame_id, value), result, None, IconKind.RESULT_FRAME)
                    step.appendRow(frame)
                    for field in fields_for(result.fields, step_id, frame_id):
                        block = self._item(field.name, result, display_field(field, "Magnitude"), IconKind.FIELD)
    ...
    def _context_menu(self, position):
        ...
        result = index.data(Qt.ItemDataRole.UserRole) if index.isValid() else None
        if result is None:
            return
